...
% %% 1. DEFINE PARAMETERS                   Instead defined in main.m
...
% Df such that Df*V=dVf and Db such that Db*V=dVb
    ...
    G.income = p.zz + p.r.*G.k; 

    left_bound  = p.mu(G.income(G.k == p.kmin, :));
    right_bound = p.mu(G.income(G.k == p.kmax, :));
    for j = 1:2
...
g_sum = g_stacked'*ones(2*G.J,1)*G.dk;
...
figure;
set(gca, 'FontSize', 18)
plot(G.k, adot, G.k, zeros(1,G.J), '--k', 'LineWidth', 2)
grid
xlabel('Capital, k', 'FontSize', 14)
